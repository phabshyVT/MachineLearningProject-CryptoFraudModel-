...
# ============================================================
# Output Logger - Writes to both console and file
# ============================================================
...
def main():
    ...
    try:
        ...
        print("\n" + "="*40)
        print("SUMMARY OF RESULTS ACROSS 3 RUNS:")
        print("="*40)
        
        # Calculate average test accuracy across runs
        test_accs = []
        for R in range(Runs):
            # Re-run evaluation for summary (or use stored results)
            pass  # We'll calculate from the last run's results
        
        # Get final test results
        Pred, Prob, Labels = evaluate(Model, DataObj, DataObj.test_mask)
        final_acc = accuracy_score(Labels, Pred)
        
        print(f"\nAverage Test Accuracy: ~{final_acc*100:.0f}%")
        print(f"Images saved to: images/gat_results_mean_std.png")
